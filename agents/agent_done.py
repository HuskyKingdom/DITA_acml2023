...
class ThorAgent:
    ...
    def action(self, model_options, training, demo=False):
        ...
        if not self.next_done:
            model_input, out = self.eval_at_state(model_options)
            self.hidden = out.hidden
            prob = F.softmax(out.logit, dim=1)
            action = prob.multinomial(1).data             #tensot[[0-5]]
            log_prob = F.log_softmax(out.logit, dim=1)
            self.last_action_probs = prob
            entropy = -(log_prob * prob).sum(1)
            log_prob = log_prob.gather(1, Variable(action))
            self.reward, self.done, self.info, self.next_done = self.episode.step(action[0, 0])
        else:
            model_input, out = self.eval_at_state(model_options)
            self.hidden = out.hidden
            prob = F.softmax(out.logit, dim=1)
            action_1 = prob.multinomial(1).data
            action_2 = torch.tensor(DONE_ACTION_INT).view(1,-1) #choose DONE by default
