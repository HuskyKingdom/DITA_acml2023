...
def data_loader(obj_file, action):
    ...
    with open(action, 'r') as f:
        for line in f.readlines():
            ...
            if skip_flag == 2 and not line.startswith('Scene'):
                continue

            if(line == "\n" or line.startswith('OBJ')):
                continue
            
            if skip_flag == 1:
                if line.startswith('['):
                    new_infe.append(line)
                if line.startswith('NONE'):
                    new_infe.append("NONE")
                skip_flag += 1
                continue
            else:
                if line.startswith('['):
            ...
            if line.startswith('Scene'):
                ...
                new_list.append(Obj)
            elif line.startswith('{'): # changing position for new format & adding reward vis
                reward = line.split('|')[1][:-1]
                rest = line.split('|')[0]
                new_list.append(rest.split(':')[1][2:-2]+"|"+reward)
            ...
            if 'Done' in new_list[-1] or len(new_list) >= 100:
                total_list.append(new_list)
                i += 1
                skip_flag = 1
                model_list.append(new_infe)
    ...
    return obj_list, total_list, model_list
...
def img_bbx(args, event, action, obj_list, ep1, target_parents,infe):
    """
    ...
    img2 = cv2.putText(img2, 'Model Infe: ', (20, 350), font, 1, black, 2)
    img2 = cv2.putText(img2, infe, (20, 420), font, 1, black, 2)
...
def start_controller(args, ep1, obj_list, target_parents,infe):
    ...
    for i in range(len(ep1)-5): # ['FloorPlan225', 'Vase', ['-2.50', '0.906817555', '2.75', '45', '0'], 'False', 'RotateLeft', 'RotateLeft', 'RotateLeft', 'RotateLeft', 'RotateLeft', 'MoveAhead', 'MoveAhead', 'MoveAhead', 'LookDown', 'RotateLeft', 'Done']
        print(ep1[i+4])
        time.sleep(0.5)
        img = img_bbx(args, event, ep1[i+4], obj_list, ep1, target_parents,infe[i+1])
        pos = event.metadata['agent']['position']
        rot = event.metadata['agent']['rotation']
        
        if 'RotateLeft' in ep1[i+4]:
            event = controller.step(dict(action='TeleportFull', x=pos['x'], y=pos['y'], z=pos['z'],
                                 rotation=rot['y']-45, horizon = angle))
            
        elif 'RotateRight' in ep1[i+4]:
            event = controller.step(dict(action='TeleportFull', x=pos['x'], y=pos['y'], z=pos['z'],
                                 rotation=rot['y']+45, horizon = angle))
        elif 'LookDown' in ep1[i+4]:
            event = controller.step(dict(action=ep1[i+4].split("|")[0]))
            angle += 30
            angle = np.clip(angle,-60,60)
        elif 'LookUp' in ep1[i+4]:
            event = controller.step(dict(action=ep1[i+4].split("|")[0]))
            angle -= 30
            angle = np.clip(angle,-60,60)
        else:
            event = controller.step(dict(action='TeleportFull', x=pos['x'], y=pos['y'], z=pos['z'],rotation=rot['y'], horizon = angle))
            event = controller.step(dict(action=ep1[i+4].split("|")[0]))
        

        frames.append(img)
        time.sleep(1.5)
    img = img_bbx(args, event, ep1[-1], obj_list, ep1, target_parents,infe[-1])
    frames.append(img)
    time.sleep(3)
    controller.stop()
...
def export_video(frames):
    ...
    video = cv2.VideoWriter('30.mp4',fourcc,1.2,(width,height))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode', type=int, default=-6, help="episode to visualize.")
    parser.add_argument('--actionList', type=str, default="action_obj_r.log", help="action log.")
    parser.add_argument('--export_video', default=True, action='store_true')
    parser.add_argument('--show_all_obj', default=False, action='store_true')
    args = parser.parse_args()
    print(args)

    obj_file = 'all_objects_v1.txt'
    obj_list, total_list, model_list = data_loader(obj_file, args.actionList)
    
    c2p = json.load(open('c2p_top3.json','r'))


    episode = total_list[args.episode]
    episode_infe = model_list[args.episode]
    target_parents = target_parent(episode, c2p)
    frames = start_controller(args, episode, obj_list, target_parents,episode_infe)
